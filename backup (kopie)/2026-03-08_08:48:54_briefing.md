...
### Swap optimalizace — TODO (čeká na sudo v terminálu)
```bash
# Snížit swappiness (ZFS doporučení)
sudo sysctl vm.swappiness=10
sudo sysctl vm.vfs_cache_pressure=50
echo "vm.swappiness=10" | sudo tee -a /etc/sysctl.d/99-ai-civilization.conf
echo "vm.vfs_cache_pressure=50" | sudo tee -a /etc/sysctl.d/99-ai-civilization.conf

# Omezit ZFS ARC na 2 GB (místo 3.8 GB)
echo "options zfs zfs_arc_max=2147483648" | sudo tee /etc/modprobe.d/zfs.conf
sudo sh -c "echo 2147483648 > /sys/module/zfs/parameters/zfs_arc_max"

# Přidat 6 GB swapfile
